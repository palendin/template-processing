...
import pandas as pd
import os
from calculation import hydroxyproline_assay_calc, recalculate
import os, sys
import warnings
...
def processing(root_directory):
    ...
    for folder_name in os.listdir(root_directory):
        ...
        # join sample layout and abs measurements together
        final_layout = pd.concat([layout_df, combined_abs_df], axis=1)

        # add QC column for user to accept/reject for reprocessng
        final_layout['data check'] = ''
        ...
        raw_data_combined.to_csv(folder_path + '/' + 'combined_raw_data.csv')
...
def reprocessing(root_directory):
    ...
    for folder_name in os.listdir(root_directory):
        ...
        # recalculate (rp = reprocessed)
        biospsy_result_rp = recalculate(raw_data_combined,folder_path)
