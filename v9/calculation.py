...
def calculate_sample_averages(combined_raw_data):
    ...
    if len(combined_raw_data[combined_raw_data['sample_type'] == 'sample']) > 0:
        ...
        if len(liquid_samples) > 0:
            ...
            # calculate std
            avg_mg_per_biopsy_std = liquid_samples.groupby(['HP_sid','experiment_ID','sample_ID', 'biopsy_id','biopsy_replicate'], squeeze=True).apply(lambda x: ((x['mg/biopsy'].sum()- x['mg/biopsy'].max()-x['mg/biopsy'].min())) if len(x) > 2 else x['mg/biopsy'].std()).reset_index(name='mg/biopsy std')
            avg_mg_per_ml_std = liquid_samples.groupby(['HP_sid','experiment_ID','sample_ID', 'biopsy_id','biopsy_replicate'], squeeze=True).apply(lambda x: ((x['mg/ml'].sum()- x['mg/ml'].max()-x['mg/ml'].min())) if len(x) > 2 else x['mg/ml'].std()).reset_index(name='mg/ml std')
            avg_mg_per_cm2_std = liquid_samples.groupby(['HP_sid','experiment_ID','sample_ID','biopsy_id','biopsy_replicate'], squeeze=True).apply(lambda x: ((x['mg/cm2'].sum()- x['mg/cm2'].max()-x['mg/cm2'].min())) if len(x) > 2 else x['mg/cm2'].std()).reset_index(name='mg/cm2 std')
        ...
        if len(solid_samples) > 0:
            ...
            # calculate std
            avg_mg_per_biopsy_std = solid_samples.groupby(['HP_sid','experiment_ID','sample_ID', 'biopsy_id','biopsy_replicate','biomaterial_id'], squeeze=True).apply(lambda x: ((x['mg/biopsy'].sum()- x['mg/biopsy'].max()-x['mg/biopsy'].min())) if len(x) > 2 else x['mg/biopsy'].std()).reset_index(name='mg/biopsy std')
            avg_mg_per_ml_std = solid_samples.groupby(['HP_sid','experiment_ID','sample_ID', 'biopsy_id','biopsy_replicate','biomaterial_id'], squeeze=True).apply(lambda x: ((x['mg/ml'].sum()- x['mg/ml'].max()-x['mg/ml'].min())) if len(x) > 2 else x['mg/ml'].std()).reset_index(name='mg/ml std')
            avg_mg_per_cm2_std = solid_samples.groupby(['HP_sid','experiment_ID','sample_ID','biopsy_id','biopsy_replicate','biomaterial_id'], squeeze=True).apply(lambda x: ((x['mg/cm2'].sum()- x['mg/cm2'].max()-x['mg/cm2'].min())) if len(x) > 2 else x['mg/cm2'].std()).reset_index(name='mg/cm2 std')
