...
def hydroxyproline_assay_calc(df,save_img_path):
    ...
    r_squared = gelatin_model.score(avg_conc_per_control[['std_conc_ug_per_well']], avg_conc_per_control[['ug/well']])
    slope = np.asscalar(np.array(gelatin_model.coef_))
    intercept = np.asscalar(np.array(gelatin_model.intercept_))

    control['r_squared'] = r_squared
    ...
    samples['ug/cm2'] = samples['mg/biopsy']/(np.pi*(pd.to_numeric(samples['biopsy_diameter_mm'])/2)**2)*1000

    # average mg/biopsy and ug/area per sample. each sample has 4 replicates. the avg exclude min and max value
    average_per_sample = samples.groupby(['experiment_ID','sample_ID', 'hide_ID','biopsy_replicate','scaffold_type'], squeeze=True).apply(lambda x: ((x['mg/biopsy'].sum()- x['mg/biopsy'].max()-x['mg/biopsy'].min())/2)).reset_index(name='mg/biopsy')
    average_mg_cm2_per_sample = samples.groupby(['experiment_ID','sample_ID'], squeeze=True).apply(lambda x: ((x['ug/cm2'].sum()- x['ug/cm2'].max()-x['ug/cm2'].min())/2)).reset_index(name='ug/cm2')
    average_per_sample['ug/cm2'] = average_mg_cm2_per_sample['ug/cm2']
    ...
    weight_per_area_mean = average_per_sample.groupby(['experiment_ID','hide_ID','scaffold_type'], squeeze = True).apply(lambda x: x['ug/cm2'].mean()).reset_index(name='ug/cm2 mean')
    weight_per_area_std = average_per_sample.groupby(['experiment_ID','hide_ID','scaffold_type'], squeeze = True).apply(lambda x: x['ug/cm2'].std()).reset_index(name='ug/cm2 std')

    # average weight of each hide_ID base on scaffold_type
    biopsy_weight = samples.groupby(['hide_ID','scaffold_type'], squeeze = True).apply(lambda x: x['net weight mg'].mean()).reset_index(name='net weight mg')
    ...
    biopsy_mean['% percent_collagen_std'] = (biopsy_mean['mg/biopsy std']/(biopsy_mean['% hydroxyproline_in_collagen']/100))/(biopsy_mean['net weight mg']-biopsy_mean['scaffold weight mg'])*100
    #print(biopsy_mean)
